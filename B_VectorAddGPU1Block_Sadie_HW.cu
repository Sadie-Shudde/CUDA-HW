...
void setUpDevices()
{
	BlockSize.x = 1024; //This will not work for the N = 1500 run because a Blocks max size is 1024 or 2^10
	BlockSize.y = 1;
	BlockSize.z = 1;
...
{
	int id = threadIdx.x; //my number
	
	
	//Since there are fewer threads then elements each thread is told 
	//to finish their work then go 1024 steps forward and see if there 
	//is another task to do. If yes, theydo that one too. 
	//If they go past the alst task they are done.
	while(id < n) //is there a task with my number
	{
		c[id] = a[id] + b[id]; //do my task with my number
		id += blockDim.x; //walk forward 1024 steps or the size of BloackSize.x
	}
}
