...
// nvcc I_DotProductManyBlocksSharedMemory_Sadie_HW9.cu -o temp
/*
 What to do:
...
/*
 ...
 1. Changed N Size for testing
 2. Change blockSize.x to 256 and change GridSize.x to the formula to calculate number of blocks needed
 3. In dotProductGPU add shared C array to store values in
 4. Have global ID and tid which is the index within this blocks shared memory
 5. Pad the unused variables with 0 will only happen in the last block
 6. In the while loop change all the id to tid so its not calculating the global thread but the one in shared memory.
 7. Add an if statement that id the tid=0 it will write that bloack's total to the global memory.
 8. Commented out the if statement that wouldn't let the vector be larger than block so I could test
 9. Added the third parameter to dotProductGPU call for the shared memory
 10.Set DotGPU to 0.0; and add up all the block values in a for loop
*/
...
// Defines
#define N 525600 // Length of the vector
...
__global__ void dotProductGPU(float *a, float *b, float *c, int n)
{
	...
	int tid = threadIdx.x; //Index within this block's shared memory
	
	//Pad the unused variables with 0 will only happen in the last block
	if(id<n)
	{
	...
	__syncthreads();//Equality might be fake but still sync your threads... like the Titanic
	//Which by the way would not be equality because more men died than women which according to feminism
	//I should have the same right to die in frigid water as a man but whatever. Hilary wouldn't have done this!
	//#WomenHaveTheRightToFreeze
	...
	//Thread 0 writes this block's total out to global memory
	if(tid == 0)
	{
		c[blockIdx.x] = C_Shared[0];
		//each block should have the final result in C_Shared[0]
		//So copy that value and send it to the corresponding block index of C_CPU
	}
}
...
int main()
{
	...
	cudaErrorCheck(__FILE__, __LINE__);
	//Third parameter is the shared memory size so it scales
