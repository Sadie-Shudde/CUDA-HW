...
// nvcc D_VectorAddFixedBlocksAndGrid_Sadie_HW4.cu -o temp
/*
 What to do:
...
/*
 ...
 3. Mess around wiht other values for block, grid, and vector size to learn the code (Submit with the hard coded values mentioned in instructions)
*/

/*
 Explain what you did to fix the code:
 1. Lines 96 and 100 hard code the BlockSize and the GridSize
 2. Lines 137-138 Change the addVectorCPU() function to the complicated addition
 3. Line 147 Add the stride variable. Stride is the total number of threads in the entire grid(total number of workers) Its the skip ahead thing from before
 4. Lines 149-157 This has the grid stride loop where a single thread will do its job then jump ahead to see what else it needs to do also the if statement inside to double check you arent leaving your designated area
 5. Line 149 #pragma unroll tells the compiler to do a few iterations back to back before checking the loop conditions again
*/
...
void setUpDevices()
{
	...
	GridSize.x = 64; // Hard coded per instructions
	GridSize.y = 1;
	GridSize.z = 1;
}
...
void addVectorsCPU(float *a, float *b, float *c, int n)
{
	...
	{ 
		c[id] = sqrtf(cosf(a[id])*cosf(a[id]) + a[id]*a[id] + sinf(a[id])*sinf(a[id]) - 1.0f)
...
__global__ void addVectorsGPU(float *a, float *b, float *c, int n)
{
	...
	int stride = blockDim.x * gridDim.x; //How much
	
		#pragma unroll 
		for(int i = id; i < n; i+=stride)
		{
			if(i<n) //Double check that id is less than n no pooping in peoples yards
			{
