...
// nvcc H_DotProductGPU1Block_Sadie_HW8.cu -o temp
/*
 What to do:
...
{
	int id = threadIdx.x; //Since we are in one block we do not need the global ID equation
	
	//First each thread multiplies one pair of numbers
	if(id<n)//Do not poop in peoples yard
	{
		c[id] = a[id] * b[id];
	}
	__syncthreads(); //Tell all the threads to wait for each other to catch up. EQUALITY
	
	//Next fold the array in half over and over until there is only 1 number left
	int size = n;
	while(size>1)
	{
		int half = size/2; //How many pairs we can make this round
		if(id<half)
		{
			c[id] += c[id+half]; //add the second half onto the first half
		}
		__syncthreads(); //Tell all the threads to wait for each other to catch up. EQUALITY
		
		//If there is left over numbers i.e. it was odd when you folded make Emersons id take on the extra work so Hayden can keep smoking
		if(size % 2 == 1 && id ==0)
		{
			c[0] += c[size-1];
		}
		...
		size = half; //The array we care about is now half as big
	}
}
